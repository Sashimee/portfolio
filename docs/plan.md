...
| **R32** | **Le reste du lot n'a toujours pas été vu dans un navigateur.** La mesure d'audience est confirmée (voir ci-dessous), mais les données structurées, les nouvelles descriptions de fiches et le pied de page rendu visible sur `/projects/:shortcode` et `/404` ne tiennent que par 86 tests et un `build` — pas par un affichage. | `curl -A "facebookexternalhit/1.1"` sur un article, qui rende son propre `og:title` et son bloc JSON-LD, puis ce bloc passé au *Rich Results Test* de Google. Et les deux pages ouvertes pour constater que le pied de page s'y voit. Recoupe R24. |
