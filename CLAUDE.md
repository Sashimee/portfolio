...
Portfolio personnel : projets, blog, contact. Quasar 2 (Vue 3 + Vite), trilingue EN/FR/DE,
...
1. **Le contenu est une donnée, jamais du balisage.** Les projets vivent dans
   ...
   texte visible** dans `src/i18n/{en,fr,de}/`. Une chaîne écrite en dur dans une composante
   n'est pas un raccourci : elle échappe au multilinguisme, et elle ne se retrouve plus.
   Ces listes ont toutes vécu dupliquées dans les pages ; c'est de là qu'elles viennent.

2. **EN, FR et DE portent exactement les mêmes clés, et aucune n'est vide.**
   `test/i18n.spec.js` le refuse, et il itère sur `AVAILABLE_LOCALES` : ajouter une langue
   à cette liste suffit à la faire garder. Une clé ajoutée l'est donc **trois** fois, dans
   la foulée — pas « en anglais d'abord, le reste suivra ».
...
- `test/i18n.spec.js` — mêmes clés dans les trois langues, aucune vide ; `resolveLocale`
  rabat « de-DE », « fr_BE » et consorts sur la langue de base.
- `test/routes.spec.js` — chaque route nommée résout ; noms de projets uniques et `img`
  / `infoKey` présents ; slugs d'articles uniques et clé i18n existante dans toutes les
...
2. La clé `key` dans les trois bundles i18n : `title`, `title2` (le sous-titre, qui admet du
